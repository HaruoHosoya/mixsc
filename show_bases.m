function h=show_bases(net,varargin)
...
pr.addParamValue('panel_titles','unit');
...
pr.addParamValue('onlyextreme',true,@islogical);
pr.addParamValue('axisticks',true,@islogical);

pr.addParamValue('fitted',false,@islogical);
pr.addParamValue('fit_type','',@isstr);

pr.addParamValue('bgimages',[],@isnumeric);

pr.addParamValue('gray_scale',false,@islogical);
...
if pr.gray_scale colormap(flipped_gray); end;
...
for K=1:nunits
    ...
    if ~isnan(pr.fontsize) set(gca,'fontsize',pr.fontsize); end;
    if ~isempty(pr.bgimages)
        imagesc(pr.bgimages(:,:,:,K));
%         colormap(gray);
    end;
    ...
    linex=[]; liney=[]; cdata=[];
    for J=1:length(CX)
        if (isnan(CX(J)) ||isnan(FR(J)) || FR(J)<pr.min_freq || abs(W(J)*mag)<min_weight || (~show_simple && F1F0(J)>1)) continue; end;
        if (W(J)>0)
...
%             set(p,'FaceVertexCData',cdata,'CDataMapping','direct','EdgeColor','flat','FaceColor','none','LineWidth',2);
    ...
    if pr.axislabels
        if disp_y==disp_height xlabel(['x [' metric ']']); end;
        if disp_x==1 ylabel(['y [' metric ']']); end;
    end;
    if disp_x~=1 set(gca,'YTickLabel',[]); end;
    if disp_y~=disp_height set(gca,'XTickLabel',[]); end;
    set(gca,'FontName','Times','FontSize',12)
    if pr.gray_scale set(gca,'color',[0.5 0.5 0.5]); end;
    axis square;
    switch panel_titles
        case 'none'
        case 'node-unit'
